...
function HF1PropPlacement.Place(model, existing)
    ...
    local locked = existing ~= nil
    local cancelled, finished = false, false

    lib.showTextUI('[LMB] Set/lock  [WASD] Move  [Q/E] Height  [←/→] Yaw  [↑/↓] Pitch  [Z/X] Roll  [G] Ground  [Shift/Ctrl] Speed  [Enter] Save  [Backspace] Cancel', {
        position = 'top-center',
        icon = 'cube',
    })
    ...
    while not finished and not cancelled do
        ...
        if not locked then
            ...
            if IsDisabledControlJustPressed(0, 24) then locked = true end
        end
        ...
        if IsDisabledControlPressed(0, 32) then delta = delta + forward * move; locked = true end
        if IsDisabledControlPressed(0, 33) then delta = delta - forward * move; locked = true end
        if IsDisabledControlPressed(0, 35) then delta = delta + right * move; locked = true end
        if IsDisabledControlPressed(0, 34) then delta = delta - right * move; locked = true end
        if IsDisabledControlPressed(0, 38) then delta = delta + vector3(0.0, 0.0, vert); locked = true end
        if IsDisabledControlPressed(0, 44) then delta = delta - vector3(0.0, 0.0, vert); locked = true end
        ...
        if IsDisabledControlPressed(0, 174) then rz = rz + turn; locked = true end
        if IsDisabledControlPressed(0, 175) then rz = rz - turn; locked = true end
        if IsDisabledControlPressed(0, 172) then rx = rx + turn; locked = true end
        if IsDisabledControlPressed(0, 173) then rx = rx - turn; locked = true end
        if IsControlPressed(0, 20) then ry = ry + turn; locked = true end -- Z
        if IsControlPressed(0, 73) then ry = ry - turn; locked = true end -- X
